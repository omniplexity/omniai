...
import { setRuntimeConfig, getRuntimeConfig, loadRuntimeConfig, normalizeBaseUrl } from '../runtimeConfig';

describe('Runtime Config', () => {
  beforeEach(() => {
    // Reset config before each test
    (global as any)._cfg = null;
  });
  ...
  describe('loadRuntimeConfig', () => {
    ...
    it('should throw if file not found', async () => {
      (global as any).fetch = async () => new Response('Not Found', { status: 404 });
    ...
    it('should throw if BACKEND_BASE_URL missing', async () => {
      (global as any).fetch = async () => new Response('{}', { status: 200 });
