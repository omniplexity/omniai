...
grep -qi "^Access-Control-Allow-Origin: ${ORIGIN}$" "${headers_opts}" || fail "Missing/invalid Access-Control-Allow-Origin"
grep -qi "^Access-Control-Allow-Credentials: true$" "${headers_opts}" || fail "Missing Access-Control-Allow-Credentials: true"
grep -qi "^Vary: .*Origin" "${headers_opts}" || fail "Missing Vary: Origin"
grep -qi "^Access-Control-Allow-Methods: .*POST.*OPTIONS" "${headers_opts}" || fail "Allow-Methods missing POST/OPTIONS"
grep -qi "^Access-Control-Allow-Headers: .*content-type" "${headers_opts}" || fail "Allow-Headers missing content-type"
grep -qi "^Access-Control-Allow-Headers: .*x-csrf-token" "${headers_opts}" || fail "Allow-Headers missing x-csrf-token"
