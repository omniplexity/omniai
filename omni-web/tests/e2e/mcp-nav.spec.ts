...
test("left nav MCP Browser opens center panel and is not rendered in right sidebar", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "MCP Browser" }).click();
  await expect(page.locator(".center-content").getByText("MCP Browser")).toBeVisible();
  await expect(page.locator(".center-content").getByRole("button", { name: "Register Server" })).toBeVisible();
